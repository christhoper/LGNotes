...
@implementation HDWebImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<HDWebImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(HDWebImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFRelease(imageDestination);
    HDAnimatedImageRep *imageRep = [[HDAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<HDWebImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            HDWebImageFrame *frame = [HDWebImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            HDWebImageFrame *frame = [HDWebImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
