...
typedef NS_ENUM(NSUInteger, HDImageCacheConfigExpireType) {
    /**
     * When the image is accessed it will update this value
     */
    HDImageCacheConfigExpireTypeAccessDate,
    /**
     * The image was obtained from the disk cache (Default)
     */
    HDImageCacheConfigExpireTypeModificationDate
};

@interface HDImageCacheConfig : NSObject
...
/**
 * The option to control weak memory cache for images. When enable, `HDImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) HDImageCacheConfigExpireType diskCacheExpireType;
